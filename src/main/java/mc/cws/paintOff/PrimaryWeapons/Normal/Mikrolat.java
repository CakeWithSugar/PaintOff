...
public class Mikrolat {
    private static PaintOffMain plugin;
    public static void setPlugin(PaintOffMain plugin) {Mikrolat.plugin = plugin;}

    public static int requiredPoints = (UltPoints.generalPoints);
    public static String reichweite = "10m";
    ...
    public static int slot = 24;

    public static void getItem(Player player) {
        ItemStack carrot = new ItemStack(Material.IRON_AXE, 1);
        ItemMeta meta = carrot.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(ChatColor.DARK_PURPLE + "Mikrolat");
    ...
    public static ItemStack arsenalDisplay() {
        ItemStack paintball = new ItemStack(Material.IRON_AXE, 1);
        ItemMeta meta = paintball.getItemMeta();
        meta.setDisplayName(ChatColor.YELLOW + "Mikrolat");
        List<String> lore = new ArrayList<>();
        lore.add(ChatColor.GRAY + "Mehr Reichweite, mehr Party!");
        lore.add(ChatColor.GRAY + "Braucht: " + ChatColor.YELLOW + requiredPoints + ChatColor.GRAY + " Ult-Punkte.");
        lore.add("");
        lore.add(ChatColor.DARK_PURPLE + "Reichweite: " + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + reichweite);
        lore.add(ChatColor.DARK_PURPLE + "Verbrauch: " + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + verbrauch + " Farbe/Schuss");
...
// Get total experience points
        ...
        if (FulePoints.isAbleToPerform(player, verbrauch)) {
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                Painter.explosionAlgorithm(snowball.getLocation().getBlock(), player, n, explosion, color, 2);
