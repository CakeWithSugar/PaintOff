...
public class TulMusket {
    private static PaintOffMain plugin;
    public static void setPlugin(PaintOffMain plugin) {TulMusket.plugin = plugin;}
    ...
    public static void getItem(Player player) {
        ItemStack carrot = new ItemStack(Material.DIAMOND_PICKAXE, 1);
        ItemMeta meta = carrot.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(ChatColor.DARK_PURPLE + "Tulipa Musket");
    ...
    public static ItemStack arsenalDisplay() {
        ItemStack paintball = new ItemStack(Material.DIAMOND_PICKAXE, 1);
        ...
        lore.add(ChatColor.DARK_PURPLE + "Cool-Down: " + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + coolDownCounter + " Sekunden");
        lore.add("");
        lore.add(ChatColor.YELLOW + "Sekundärwaffe: " + ChatColor.LIGHT_PURPLE + "Klotzmine");
        lore.add(ChatColor.YELLOW + "Spezialwaffe: " + ChatColor.LIGHT_PURPLE + "Lauffeuer");
        meta.setLore(lore);
    ...
    public static void shoot(Player player) {
        if (FulePoints.isAbleToPerform(player, verbrauch)) {
            ...
            Vector leftDirection = direction.clone().rotateAroundY(Math.toRadians(5));
            Vector rightDirection = direction.clone().rotateAroundY(Math.toRadians(-5));
            snowball.setVelocity(leftDirection.multiply(1.25));
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                snowball2.setVelocity(direction.multiply(1.25));
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                Snowball snowball3 = player.launchProjectile(Snowball.class);
                snowball3.setVelocity(rightDirection.multiply(1.25));
                Game.applyGravity(snowball3, 0.05);
                snowball3.setMetadata("spawnedBy", new FixedMetadataValue(plugin, "Lower"));
                SnowballEffect.sin(snowball3, player, Start.getQueueNumber(player));
            }, spawnDelay*2L);
