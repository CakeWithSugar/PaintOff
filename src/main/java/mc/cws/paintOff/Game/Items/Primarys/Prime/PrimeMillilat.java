package mc.cws.paintOff.Game.Items.Primarys.Prime;
...
public class PrimeMillilat {
    private static PaintOffMain plugin;
    public static void setPlugin(PaintOffMain plugin) {
        PrimeMillilat.plugin = plugin;}

    public static int requiredPoints = (UltPoints.generalPoints + 30);
    public static int verbrauch = 12;
    public static int explosion = 1;
    public static int coolDown = 2;
    public static int slot = 33;
    private static final long cancelTime = 10L; // 20 ticks = 1 second
    ...
    public static void getItem(Player player) {
        ItemStack carrot = new ItemStack(Material.GOLDEN_AXE, 1);
        ItemMeta meta = carrot.getItemMeta();
        if (meta != null) {
            meta.setDisplayName(ChatColor.DARK_PURPLE + "Prime Millilat");
    ...
    public static ItemStack arsenalDisplay() {
        ItemStack paintball = new ItemStack(Material.GOLDEN_AXE, 1);
        ItemMeta meta = paintball.getItemMeta();
        meta.setDisplayName(ChatColor.YELLOW + "Prime Millilat");
        List<String> lore = new ArrayList<>();
        lore.add(ChatColor.GRAY + "Vergrößerte Reichweite und Standhaftigkeit!");
        lore.add(ChatColor.GRAY + "Braucht: " + ChatColor.YELLOW + requiredPoints + ChatColor.GRAY + " Ult-Punkte.");
        lore.add("");
        lore.add(ChatColor.DARK_PURPLE + "Reichweite: " + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + reichweite);
        lore.add(ChatColor.DARK_PURPLE + "Verbrauch: " + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + verbrauch + " Farbe/Schuss");
        ...
        lore.add(ChatColor.DARK_PURPLE + "Cool-Down: " + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + coolDownCounter + " Sekunden");
        lore.add("");
        lore.add(ChatColor.YELLOW + "Sekundärwaffe: " + ChatColor.LIGHT_PURPLE + "Dreifärber");
        lore.add(ChatColor.YELLOW + "Spezialwaffe: " + ChatColor.LIGHT_PURPLE + "Sonnenwindler");
        meta.setLore(lore);
...
// Get total experience points
        ...
        if (FulePoints.isAbleToPerform(player, verbrauch)) {
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                Painter.explosionAlgorithm(snowball.getLocation().getBlock(), player, n, explosion, color, 3);
